...
OBJECT *getVisible(const char *intention, const char *noun)
{
    ...
    {
        printf("Please be more specific about which %s you mean.\n", noun);
        obj = NULL;
    }
...
OBJECT *getPossession(OBJECT *from, const char *verb, const char *noun)
{
    ...
    else if ((obj = getObject(noun, player, distHeldContained)) == NULL)
    {
        if (getObject(noun, player, distNotHere) == NULL)
        {
    ...
    {
        printf("Please be more specific about which %s you want to %s.\n", noun, verb);
        obj = NULL;
    }
    else if (obj == from)
    {
